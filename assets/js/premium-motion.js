(() => {
  ...
  function setupPremiumCursor() {
    ...
    $$('.btn,.contact-item,.filter-btn,.slide-btn,.assistant-launch').forEach((el) => {
  ...
  function addRecruiterAssistant() {
    ...
    const submit = () => {
      ...
      if (!value) return;
      log.insertAdjacentHTML('beforeend', `<div class="assistant-msg user">${escapeHTML(value)}</div><div class="assistant-msg">${escapeHTML(answerQuestion(value))}</div>`);
      input.value = '';
  ...
  function addCommandPalette() {
    ...
    const render = () => {
      ...
      list.innerHTML = commands.filter(([name]) => name.toLowerCase().includes(query)).map(([name, target]) => `<div class="command-item" data-target="${target}"><span>${name}</span><span>↵</span></div>`).join('');
    };
    ...
    list.addEventListener('click', (event) => { const item = event.target.closest('.command-item'); if (item) run(item.dataset.target); });
    input.addEventListener('keydown', (event) => {
      if (event.key === 'Escape') close();
      if (event.key === 'Enter') $('.command-item', list)?.click();
    });
  ...
  function openTerminal() {
    ...
    if (!overlay) {
      ...
      document.body.appendChild(overlay);
      const output = $('.terminal-output', overlay);
      ...
      input.addEventListener('keydown', (event) => {
        ...
        else output.textContent += `\n> ${command}\n${map[command] || 'Unknown command. Try whoami, experience --list, contact --email, open linkedin.'}`;
      });
  ...
  function openTools() {
    ...
    if (!overlay) {
      ...
      document.body.appendChild(overlay);
      const calculate = () => {
        const start = Number($('#calcStart')?.value || 0);
        const end = Number($('#calcEnd')?.value || 0);
        const days = Number($('#calcDays')?.value || 0);
        $('#calcAdg').textContent = days > 0 ? ((end - start) / days).toFixed(2) : '0.00';
      };
      ['calcStart', 'calcEnd', 'calcDays'].forEach((id) => $('#' + id, overlay)?.addEventListener('input', calculate));
      overlay.addEventListener('click', (event) => { if (event.target === overlay) overlay.classList.remove('open'); });
    }
    overlay.classList.add('open');
  }
